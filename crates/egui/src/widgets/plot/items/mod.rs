...
pub struct HLine {
    ...
    /// Whether first boolean is true, it allows highlighting
    /// and the second boolean is its changing state.
    pub(super) highlight: [bool; 2],
...
impl PlotItem for HLine {
    fn shapes(&self, ui: &mut Ui, transform: &ScreenTransform, shapes: &mut Vec<Shape>) {
        ...
        style.style_line(points, *stroke, highlight[1], shapes);
    }
    ...
    fn highlight(&mut self) {
        self.highlight = [true, self.highlight[1]];
    }
...
impl PlotItem for Line {
    fn shapes(&self, _ui: &mut Ui, transform: &ScreenTransform, shapes: &mut Vec<Shape>) {
        ...
        style.style_line(values_tf, *stroke, highlight[1], shapes);
    }
...
impl PlotItem for Arrows {
    fn shapes(&self, _ui: &mut Ui, transform: &ScreenTransform, shapes: &mut Vec<Shape>) {
        ...
        let stroke = Stroke::new(if highlight[0] && highlight[1] { 2.0 } else { 1.0 }, *color);
        origins
            .points()
            .iter()
...
impl PlotItem for BarChart {
    fn shapes(&self, _ui: &mut Ui, transform: &ScreenTransform, shapes: &mut Vec<Shape>) {
        for b in &self.bars {
            b.add_shapes(transform, self.highlight[1], shapes);
        }
    }
